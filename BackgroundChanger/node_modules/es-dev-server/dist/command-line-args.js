 ...
 */
function readCommandLineArgs(argv = process.argv) {
  ...
  const dashesArgs = (0, _commandLineArgs.default)(optionDefinitions, {
    argv,
    partial: true
  });
  ...
  if (args.config) {
    ...
    const configPath = _path.default.resolve(args.config);
    ...
    const defaultConfigPath = _path.default.join(process.cwd(), 'es-dev-server.config.js');
  ...
  let {
    open
  } = options; // open can be a boolean nor a string. if it's a boolean from command line args,
