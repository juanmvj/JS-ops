...
function createNodeResolveConfig(rootBaseDir, resolveDirectories, fileExtensions) {
  return {
    plugins: [[require.resolve('@open-wc/building-utils/forked-babel-plugin-bare-import-rewrite'), {
      rootBaseDir,
      alwaysRootImport: ['**'],
...
function createBabelCompiler(cfg) {
  const fileExtensions = [..._constants.baseFileExtensions, ...cfg.extraFileExtensions]; // create babel configuration based on a number of presets

  const babelConfigs = [// custom babel config if it's there
  createDefaultConfig(cfg.readUserBabelConfig), cfg.nodeResolve && createNodeResolveConfig(cfg.rootDir, cfg.moduleDirectories, fileExtensions), cfg.modern && modernConfig, cfg.legacy && legacyConfig, cfg.customBabelConfig].filter(Boolean);
  ...
  async function compile(filename, sourceCode, lastModified) {
    ...
    const compileTask = (0, _core.transformAsync)(sourceCode, {
      filename,
      ...babelConfig
    });
    ...
    try {
      const {
        code: compiledCode
      } = await compileTask; // cache result for later
